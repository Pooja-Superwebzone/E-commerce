...
function normalizeImageInput(value) {
  ...
  if (!image) return "";
  // Accept data-url images from file upload, and keep backward compatibility for URLs.
...
router.get("/products", async (req, res) => {
  try {
    ...
    let items;
    if ((featured === "true" && !Number.isNaN(parsedLimit) && parsedLimit > 0) || (!Number.isNaN(parsedLimit) && parsedLimit > 0)) {
...
router.post(
  "/products",
  requireAuth,
  requireAdminPermission("product_add"),
  async (req, res) => {
    try {
      const db = await getDb();
      const products = db.collection("products");
      ...
      if (!doc.name || !doc.category || !doc.subcategory || !doc.image) {
        return fail(
          res,
          "name, category, subcategory and image are required.",
          400
        );
      }
...
router.patch(
  "/products/:id",
  requireAuth,
  requireAdminPermission("product_edit"),
  async (req, res) => {
    try {
      const id = Number(req.params.id);
      if (!id) return fail(res, "Invalid product id.", 400);
      ...
      if (payload.category !== undefined) update.category = String(payload.category || "").trim().toLowerCase();
      if (payload.subcategory !== undefined) update.subcategory = String(payload.subcategory || "").trim().toLowerCase();
      if (payload.price !== undefined) update.price = toNumber(payload.price, 0);
      if (payload.originalPrice !== undefined) update.originalPrice = toNumber(payload.originalPrice, 0);
      ...
      if (payload.badge !== undefined) update.badge = payload.badge ? String(payload.badge) : null;
      ...
      if (payload.gstPercent !== undefined) update.gstPercent = toNumber(payload.gstPercent, 0);
      if (payload.stock !== undefined) update.stock = toNumber(payload.stock, 0);

      if (update.gstPercent !== undefined && (update.gstPercent < 0 || update.gstPercent > 100)) {
        return fail(res, "gstPercent must be between 0 and 100.", 400);
      }
