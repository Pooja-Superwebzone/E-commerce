...
const { normalizePermissions, defaultPermissionsByRole } = require("../utils/permissions");
...
router.post("/login", async (req, res) => {
  try {
    ...
    const user = {
      ...
      permissions: normalizePermissions(userDoc.permissions, userDoc.role || ROLES.USER),
    };
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const canManageAdmins = actorRole === ROLES.SUPER_ADMIN || Boolean(req.authUser.permissions?.manage_admins);
      if (!canManageAdmins && (requestedRole === ROLES.SUPER_ADMIN || requestedRole === ROLES.ADMIN)) {
        return res.status(403).json({
          message: "You do not have permission to create admin or super admin users.",
        });
      }
...
router.patch(
  ...
  async (req, res) => {
    try {
      ...
      const canManageAdmins = actorRole === ROLES.SUPER_ADMIN || Boolean(req.authUser.permissions?.manage_admins);
      if (!canManageAdmins && (targetRole === ROLES.ADMIN || targetRole === ROLES.SUPER_ADMIN)) {
        return res.status(403).json({
          message: "You do not have permission to manage admin/super admin permissions.",
        });
      }
